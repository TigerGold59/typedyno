...
export class Jumprole extends ParentCommand {
    ...
    async pre_dispatch(
        ...
        const reply = async (response: string) => {
            await interaction.reply(`${prefix}jumprole: ${response}`);
        };
        ...
        if (authorized_channels.rowCount === 0) {
            ...
            log(`jumprole: server with ID ${interaction.guild.id} has multiple jumprole commands channels.`, LogType.Error);
            ...
            if (is_valid_Snowflake(channel_id)) {
                ...
                log(
                    `jumprole: server with ID ${interaction.guild.id} returned a non-Snowflake type when queried for the authorized channel ID.`,
                    LogType.Error,
                );
